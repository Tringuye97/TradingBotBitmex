...
class BitMEXWebsocket:
    ...
    def get_instrument(self):
        ...
        return instrument
#########################################################################################
    def get_tickerXBTUSD(self):
        '''Return a ticker object. Generated from quote and trade.'''
        lastQuote = self.data['quote'][-1]
        lastTrade = self.data['trade'][-1]
        ticker = {
            "Symbol" : lastQuote['symbol'],
            "Last Trade": lastTrade['price'],
            "Bid Price": lastQuote['bidPrice'],
            "Bid Size" : lastQuote['bidSize'],
            "Ask Price": lastQuote['askPrice'],
            "Ask Size" : lastQuote['askSize'],
        }
        # The instrument has a tickSize. Use it to round values.
        # instrument = self.data['instrument'][0]
        # return {k: round(float(v or 0), instrument['tickLog']) for k, v in ticker.items()}
        # return {k: float(v or 0) for k, v in ticker.items()}
        return ticker
################# FUNDING INFORMATION #######################################################
    # def funds(self):
    ...
    def funds(self):
        '''Get account balance'''
        balance = self.data['margin'][0]
        balance_info = {
            'Wallet Balance' : balance['walletBalance']/100000000.0,
            'Margin Balance' : balance['marginBalance']/100000000.0,
            'Available Margin' : balance['availableMargin']/100000000.0,
            'Withdrawable Margin' : balance['withdrawableMargin']/100000000.0,
        }
        return balance_info
...
#######################################################################
    ...
    def place_order(self, quantity, price):
        ...
        postdict = { 'symbol': self.symbol, 'orderQty':quantity, 'price':price, 'clOrdID':clOrdID}
        return _curl_bitmex(path=endpoint, postdict=postdict, verb='POST')
        #return self.Interact(path=endpoint, postdict=postdict, verb='POST') 
